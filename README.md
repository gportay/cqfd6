...
Unless `custom_img_name` is used, Docker images generated by cqfd will be named
`cqfd_$username_$org_$name_$hash` (`cqfd_$username_$org_$name_$hash_$distro` if
`distro` is set), using the following variables:
...
* `$distro`: The `distro` variable in the `[build]` section of your `.cqfdrc`.
