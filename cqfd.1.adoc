...
cqfd6 - a tool to wrap commands in controlled Docker containers using docker.

== SYNOPSIS

*cqfd6* [OPTIONS] --init|--deinit

*cqfd6* [OPTIONS] --ls|--gc

*cqfd6* [OPTIONS] --exec COMMAND [ARGUMENTS]

*cqfd6* [OPTIONS] --flavors

*cqfd6* [OPTIONS] --run [COMMAND_STRING]

*cqfd6* [OPTIONS] --run -c ARGUMENT_STRING

*cqfd6* [OPTIONS] --shell [SHELL_ARGUMENTS]

*cqfd6* [OPTIONS] [--] [PROGRAM] [ARGUMENTS]

== DESCRIPTION

*cqfd6(1)* provides a quick and convenient way to run commands in the current
...
configured in your _Cqfdfile_ (see build.command).
...
*-f* or *--cqfd-file FILE*::
	Use file as config file (default _Cqfdfile_).
...
*CQFDFILE*::
	File used as config file (default _Cqfdfile_).
...
*CQFD_PROMPT*::
	Set to _yes_ or _no_ to set user response to prompt messages; prompt is
	disabled.
...
*CQFD_SUDO_NOPASSWD*::
	Set to _true_ to run all commands without password authentication using
	_sudo_ in the build container.

== FILES

*Cqfdfile*::
	Project information config file (See *Cqfdfile(5)*).
...
*docker-build(1)*, *docker-run(1)*, *Dockerfile(5)*, *Cqfdfile6(5)*
